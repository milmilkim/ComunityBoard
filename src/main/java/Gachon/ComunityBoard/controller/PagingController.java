...
public class PagingController {
    ...
    @GetMapping("/api/home")
    public Page<PostsListResponseDTO> paging(
            @PageableDefault(size = 12, sort = "createdDate", direction = Sort.Direction.DESC) Pageable pageRequest) {
        Page<Posts> postsList = postsRepository.findAll(pageRequest);

        Page<PostsListResponseDTO> pagingList = postsList.map(posts -> new PostsListResponseDTO(posts));
        return pagingList;
    }
    ...
    public Page<PostsListResponseDTO> searchedPaging(@PathVariable("keyword") String keyword,
            @PageableDefault(size = 12, sort = "createdDate", direction = Sort.Direction.DESC) Pageable pageRequest) {
    ...
    public void initializing() {
        ...
        for (int i = 0; i < 100; i++) {
            Posts posts = Posts.builder().title(i + "번 게시글").content(i + "내용내용").writer("호박고구마").event("야구야구")
                    .eventTime("123123").build();
            postsRepository.save(posts);
        }
        for (int i = 0; i < 13; i++) {
            Posts posts = Posts.builder().title(i + "번 게시글").content(i + "내용내용").writer("김승환").event("야구야구").build();
            postsRepository.save(posts);
        }
    }
